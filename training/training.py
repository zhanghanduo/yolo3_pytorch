    ...
    for epoch in range(config["epochs"]):
        for step, samples in enumerate(dataloader):
            ...
            if step > 0 and step % 1000 == 0:
                # net.train(False)
                _save_checkpoint_small(net.state_dict(), config, step, epoch)
                # net.train(True)

        lr_scheduler.step()

    # net.train(False)
    _save_checkpoint(net.state_dict(), config, config["epochs"])
    # net.train(True)
    logging.info("Bye~")


# best_eval_result = 0.0
def _save_checkpoint(state_dict, config, epoch_num, evaluate_func=None):
...
def _save_checkpoint_small(state_dict, config, step, epoch_num, evaluate_func=None):
    # global best_eval_result
    model_name = "model_" + str(epoch_num) + "_" + str(step) + ".pth"
    checkpoint_path = os.path.join(config["sub_working_dir"], model_name)
    torch.save(state_dict, checkpoint_path)
    logging.info("Model checkpoint saved to %s" % checkpoint_path)
